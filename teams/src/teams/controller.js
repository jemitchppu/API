...
const insertTeams = (req,res) => {
    const {id, teamnames, teamcity} = req.body;
    pool.query(queries.insertTeams,[id, teamnames, teamcity], (error, results)=>{
        if(error)throw error;
...
const updateCity = (req,res) => {
    ...
    const { teamcity, id} = req.body;
    pool.query(queries.updateCity, [ teamcity, id],(error,results)=> {
        if(error)throw error;
